    ...
    for i in range(1, args.epoch+1):
        ...
        if test:
            ...
            if args.task == 'r':
                ...
                val_metrics = res_eval['macroF1'] + res_eval['microF1']
                val_best_metrics = eval_bestRes['macroF1'] + eval_bestRes['microF1']
                if (val_metrics) > (val_best_metrics):
                    print('%s %.4f, %s %.4f' % ('Val metrics increase from', val_best_metrics, 'to', val_metrics))
                    eval_bestRes['macroF1'] = res_eval['macroF1']
                    eval_bestRes['microF1'] = res_eval['microF1']
                    update = True
                reses = engine.eval(False, True)
                torch.save(engine.model.state_dict(),
                        args.save + args.data + "/" + "_epoch_" + str(i) + "_macroF1_" + str(round(reses['macroF1'], 2)) + "_microF1_" + str(
                            round(reses['microF1'], 2)) + ".pth")
                if update:
                    print(makePrint('Test', i, reses))
                    bestRes = reses
                    update = False
