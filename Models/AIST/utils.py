...
def load_data_regions(bs, target_crime_cat, target_region, target_city, tw=120):
    ...
    for i in com:
        loaded_data = torch.from_numpy(np.loadtxt("data/" + target_city + "/12h/com_crime/r_" + str(i) + ".txt", dtype=np.float32)).T
        ...
        # test_x = test_x[:test_x.shape[0] - 11, :]               # (bs, tw): sub 11 to make it consistent with bs = (672, 120)
        # test_y = test_y[:test_y.shape[0] - 11, :]               # (bs, 1): sub 11 to make it consistent with bs = (672, 1)
...
def load_data_regions_external(bs, nxfeatures, target_region, target_city, tw=120):
    ...
    poi_data = torch.from_numpy(np.loadtxt("data/" + target_city + "/poi.txt", dtype=np.int32))

    for i in com:
        loaded_data = torch.from_numpy(np.loadtxt("data/" + target_city + "/act_ext/taxi" + str(i) + ".txt", dtype=np.int32)).T
...
def load_data_sides_crime(bs, target_crime_cat, target_region, target_city, tw=120):
    ...
    for i in range(len(com)):
        loaded_data = torch.from_numpy(np.loadtxt("data/" + target_city + "/12h/side_crime/s_" + str(side[i]) + ".txt", dtype=np.int32)).T
        loaded_data = loaded_data[:, target_crime_cat:target_crime_cat + 1]
        tensor_ones = torch.from_numpy(np.ones((loaded_data.size(0), loaded_data.size(1)), dtype=np.int32))
...
def gen_com_adj_matrix(target_region, target_city):
    adj_matrix = np.zeros((77, 77), dtype=np.int)
    edges_unordered = np.genfromtxt("data/" + target_city + "/com_adjacency.txt", dtype=np.int32)
    for i in range(edges_unordered.shape[0]):
        src = edges_unordered[i][0] - 1
        dst = edges_unordered[i][1] - 1
        adj_matrix[src][dst] = 1
        adj_matrix[src][dst] = 1
    np.savetxt("data/" + target_city + "com_adj_matrix.txt", adj_matrix, fmt="%d")
    return


def gen_com_side_adj_matrix(regions, target_city):
    idx = np.loadtxt("data/" + target_city + "/side_com_adj.txt", dtype=np.int32)
