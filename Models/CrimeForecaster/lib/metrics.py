...
def cross_entropy_loss(preds, labels):
    ...
    return tf.nn.sigmoid_cross_entropy_with_logits(labels=labels, logits=preds)
    # return tf.losses.softmax_cross_entropy(onehot_labels=labels, logits=preds)
...
def cross_entropy():
    def loss(preds, labels):
        return cross_entropy_loss(preds=preds, labels=labels)
