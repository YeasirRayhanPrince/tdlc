    ...
    def eval(self, iseval, isSparsity):
        ...
        for i in range(steps):
            ...
            if isSparsity:
                ...
                if args.task == 'r':
                    ...
                    _, truePos1, falseNeg1, trueNeg1, falsePos1 = utils.cal_metrics_r_mask(output.cpu().detach().numpy(), labels, mask, self.handler.mask1)
                    _, truePos2, falseNeg2, trueNeg2, falsePos2 = utils.cal_metrics_r_mask(output.cpu().detach().numpy(), labels, mask, self.handler.mask2)
                    _, truePos3, falseNeg3, trueNeg3, falsePos3 = utils.cal_metrics_r_mask(output.cpu().detach().numpy(), labels, mask, self.handler.mask3)
                    _, truePos4, falseNeg4, trueNeg4, falsePos4 = utils.cal_metrics_r_mask(output.cpu().detach().numpy(), labels, mask, self.handler.mask4)

                    loss, truePos, falseNeg, trueNeg, falsePos = self.metrics(output.cpu().detach().numpy(), labels, mask)
        ...
        if isSparsity == False:
            ...
            if args.task == 'r':
                ...
                # ret['microF1'] = ret['F1']
                ret['macroF1'] = np.sum(epochTruePos) * 2 / (np.sum(epochTruePos) * 2 + np.sum(epochFalsePos) + np.sum(epochFalseNeg))
                ret['microF1'] = np.mean([ret['F1_%d' % i] for i in range(args.offNum)])
