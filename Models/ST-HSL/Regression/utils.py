...
def cal_metrics_r(preds, labels, mask):
    if args.task == 'c':
        loss = F.binary_cross_entropy(preds, labels, reduction='none')
        posInd = (labels > 0).astype(np.float32)
        negInd = (labels < 0).astype(np.float32)
        posPred = (preds >= args.border).astype(np.float32)
        negPred = (preds < args.border).astype(np.float32)
        truePos = np.sum(posPred * posInd, axis=0)
        falseNeg = np.sum(negPred * posInd, axis=0)
        trueNeg = np.sum(negPred * negInd, axis=0)
        falsePos = np.sum(posPred * negInd, axis=0)

        return loss, truePos, falseNeg, trueNeg, falsePos
