...
from model.loss import cross_entropy, cal_hr_loss, mae_loss
...
class HAGENSupervisor:
    ...
    def evaluate(self, epoch, dataset='val', batches_seen=0, flag=None ):
        with torch.no_grad():
            self.hagen_model = self.hagen_model.eval()
...
#############################################################################
            ...
            if dataset in ['test_easter', 'test_xmas', 'test_upto_MARCH']:
                for i in range(77):
                    # print(y_truth[0][0][0])
                    # print(y_truth[0][0][1])
                    ...
                    y_truth_reshape_i = np.reshape(y_truth_i, (-1, self._model_kwargs['output_dim']))
                    y_pred_reshape_i = np.reshape(y_pred_i, (-1, self._model_kwargs['output_dim']))
                    y_pred_reshape_sigmoid_i = sigmoid(y_pred_reshape_i)
                    ss = MinMaxScaler(feature_range=(0, 1))
                    # print(y_pred_reshape_sigmoid_i.shape)
                    ...
                    with open(self._log_dir + "/" + str(epoch) + "/" + str(i) + "/" + dataset + "_y_pred_reshape_sigmoid.pkl", 'wb') as f:
                        pickle.dump(y_pred_reshape_sigmoid_i, f)
                    with open(self._log_dir + "/" + str(epoch) + "/" + str(i) + "/" + dataset + "_y_truth_reshape.pkl", 'wb') as f:
...
#############################################################################
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                optimizer.step()
            lr_scheduler.step()
            val_loss, update, valCE, valHR, valmacro, valmicro = self.evaluate(epoch = epoch_num, dataset='val', batches_seen=batches_seen, flag=best_performance)
            end_time = time.time()
            ...
            # test_loss, macro_test, micro_test = self.evaluate(epoch = epoch_num, dataset='test', batches_seen=batches_seen)
            ##==>
            test_loss_easter, macro_easter, micro_easter = self.evaluate(epoch = epoch_num, dataset='test_easter', batches_seen=batches_seen)
            test_loss_xmas, macro_xmas, micro_xmas = self.evaluate(epoch = epoch_num, dataset='test_xmas', batches_seen=batches_seen)
            test_loss_upto_march, macro_upto_march, micro_upto_march = self.evaluate(epoch = epoch_num, dataset='test_upto_MARCH', batches_seen=batches_seen)
            ##==>
            ...
            if update == True:
                ...
                micro_easter_best = micro_easter
                macro_easter_best = macro_easter
                micro_xmas_best = micro_xmas
                macro_xmas_best = macro_xmas
                micro_upto_march_best = micro_upto_march
                macro_upto_march_best = macro_upto_march
        ...
        self._logger.info('EASTER: For the best epoch{}: The average macro-F1 score is {:.5f}, '
                          'average micro-F1 score is {:.5f}'.format(best_epoch, macro_easter_best, micro_easter_best))
        self._logger.info('XMAS: For the best epoch{}: The average macro-F1 score is {:.5f}, '
                          'average micro-F1 score is {:.5f}'.format(best_epoch, macro_xmas_best, micro_xmas_best))
        self._logger.info('UPTO MARCH: For the best epoch{}: The average macro-F1 score is {:.5f}, '
                          'average micro-F1 score is {:.5f}'.format(best_epoch, macro_upto_march_best, micro_upto_march_best))
    ...
    def _compute_loss(self, y_true, y_predicted, loss_type='train', adj_mx=None, beta=0.01):
        ...
        # loss_seq = mae_loss(y_predicted, y_true)
        if(loss_type=='train'):
            loss_hr = 0.0
